...
public class IcoSphereGenerator
{
    ...
    {
        noDivisions = Mathf.Max(0, resolution);
        noVerticesPerFace = ((noDivisions + 3) * (noDivisions + 3) - (noDivisions + 3)) / 2;
        int noVertices = noVerticesPerFace * 8 - (noDivisions + 2) * 12 + 6;
        int noTrianglesPerFace = (noDivisions + 1) * (noDivisions + 1);

        vertices = new FixedSizeList<Vector3>(noVertices);
        triangles = new FixedSizeList<int>(noTrianglesPerFace * 8 * 3);
        ...
        for (int i = 0; i < vertexPairs.Length; i += 2)
        {
            ...
            int[] edgeVertexIndices = new int[noDivisions + 2];
            edgeVertexIndices[0] = vertexPairs[i];

            // Add vertices along edge
            for (int divisionIndex = 0; divisionIndex < noDivisions; divisionIndex++)
            {
                float t = (divisionIndex + 1f) / (noDivisions + 1f);
                edgeVertexIndices[divisionIndex + 1] = vertices.nextIndex;
                vertices.Add(Vector3.Slerp(startVertex, endVertex, t));
            }
            edgeVertexIndices[noDivisions + 1] = vertexPairs[i + 1];
            int edgeIndex = i / 2;
    ...
    void CreateFace(Edge sideA, Edge sideB, Edge bottom, bool reverse)
    {
        ...
        var vertexMap = new FixedSizeList<int>(noVerticesPerFace);
