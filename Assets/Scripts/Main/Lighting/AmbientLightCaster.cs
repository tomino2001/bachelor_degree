...
public class AmbientLightCaster : MonoBehaviour {
    ...
    void Start () {
        ...
        transform.rotation = CalculateAmbientLightRotation ();
    }

    // Do this update after all the updates
    void LateUpdate () {

        transform.rotation = Quaternion.Slerp (transform.rotation, CalculateAmbientLightRotation (), Time.deltaTime * .2f);
    ...
    Quaternion CalculateAmbientLightRotation () {
